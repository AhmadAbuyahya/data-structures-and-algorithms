...
const firstLetters = (arr) => {
  // Solution code here...
  let newArr = [];
  arr.map((value)=>newArr.push(value.substring(0,1)));
  return newArr;
};
...
const findHappiness = (arr) => {
  // Solution code here...
  let newArr=[];
  arr.map(value=>{if (value.includes(':)')){newArr.push(value);
  }});
  return newArr;
};
...
const standardizePhoneNumbers = (arr) => {
  ...
  let newArr=[];
  arr.map(value=>{
    newArr.push(`${value.substring(1,4)}${value.substring(6,9)}${value.substring(10)}`);
  });
  return newArr;
};
